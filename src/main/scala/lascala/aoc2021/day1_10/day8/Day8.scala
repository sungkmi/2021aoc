...
extension (d: Digit)
  infix def isSubsetOf(other: Digit): Boolean =
    d.forall(other.contains)
  infix def isSupersetOf(other: Digit): Boolean =
    other isSubsetOf d
...
def recognize(pattern: SignalPattern): Map[Digit, Int] =
  ...
  val six  = group(6).filterNot(_ isSupersetOf one).head
  val nine = group(6).filter(_ isSupersetOf four).head
  val zero = (group(6) - six - nine).head

  val three = group(5).filter(_ isSupersetOf one).head
  val five  = group(5).filter(_ isSubsetOf six).head
